...
void *conn_thread(void *vargp) {
  struct conn_input *inp = (struct conn_input *)vargp;
  char hostname[MAXLINE], port[MAXLINE];

  int rc;
  if ((rc = getnameinfo((SA *)(&inp->clientaddr), inp->clientlen, hostname,
...
int main(int argc, char **argv) {
  ...
  while (1) {
    inp = malloc(sizeof(struct conn_input));
    inp->clientlen = sizeof(inp->clientaddr);
    inp->connfd = accept(listenfd, (SA *)(&inp->clientaddr), &inp->clientlen);
    if (inp->connfd < 0) {
    ...
    pthread_t tid;
    if (pthread_create(&tid, NULL, conn_thread, inp) != 0)
      app_error("Failed to create thread");
    if (pthread_detach(tid) != 0)
      app_error("Failed to detach thread");
  }
}
