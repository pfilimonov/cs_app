...
void release_sig(sigset_t prev_mask) {
...
void eval(char *cmdline) {
  ...
  if (!builtin_cmd(argv)) {
    sigset_t prev_mask = block_sig(SIGCHLD);
    ...
    int res;
    if (!bg) {
      res = addjob(jobs, pid, FG, cmdline);
      if (res == 0)
        unix_error("Failed to add fg job");

    } else {
      res = addjob(jobs, pid, BG, cmdline);
      if (res == 0)
...
void do_bgfg(char **argv) {
  ...
  if ((pid = strtol(argv[1], NULL, 10)) != 0) {
    ...
    long jid = strtol(argv[1] + 1, NULL, 10);
...
void sigchld_handler(int sig) {
  ...
  if ((pid = waitpid(-1, &status, WNOHANG | WUNTRACED)) > 0) {
    ...
    struct job_t *job = getjobpid(jobs, pid);
    if (job == NULL)
      unix_error("Reap non-existend job");

    int delete = 0;
    if (WIFSIGNALED(status)) {
      char s[256];
      sprintf(s, "Job [%d] (%d) terminated by signal %d\n", job->jid, pid,
              WTERMSIG(status));
      Sio_puts(s);
      delete = 1;
    }
